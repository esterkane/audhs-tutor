...
- Projection for starting TTS at the first **clause** (comma/semicolon boundary, ≥ 3 words): the clause is ready at 1.17 s and synthesises in 0.63 s → **≈ 1.76 s median first audio**, i.e. under the target. This is first-clause boundary + its separately measured synthesis time, not a run of a modified loop — the change to `voice/loop.py` (speak per clause, then per sentence) is the next experiment, and it must keep speech natural (a clause read on its own can sound clipped).
