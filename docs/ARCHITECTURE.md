...
Curriculum workflow (P4): `curriculum_draft{course, section, status draft|published|rejected, origin deterministic|model, payload_json, validation_json, version}` (a published draft writes a *new* `learning_object` version and new `assessment` rows; `skill_node.course` marks published-from course; `skill_node.section` + `order_no` (section order × 1000 + lecture position, stamped at publish) order the goal-narrowed next-skill pick and are *not* prerequisites), `content_report{kind, turn_id, chunk_id, skill_id, note, status}`. Ingest runs (course-material stage 1): `ingest_run{src, course, options_json, status running|finished|interrupted|resumed|failed, files_total, files_done, last_uri, resumed_from, summary_json}` and `ingest_run_item{run_id, uri, outcome, reason, document_id, version_id}` — progress and resume of long imports; not learner state. Course curation (stage 3): `course_source{course, document_id (unique), role primary|supplemental|excluded, reason, decided_by, decided_at}` — the owner's decision about what a document is for a course; drafts use primary sources only.
