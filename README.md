...
The **Audio visualizer lab** link opens a local creative-coding experiment: use a silent demo or choose an audio file, inspect RMS and frequency-band levels, and edit a validated JSON graph that drives rings, bars or orbiting dots. Labeled controls let you change colors, shapes and block parameters without editing JSON; three examples provide starting points. Each block has an Explain button with a local explanation and experiment hint. An optional signal-flow diagram shows the actual connections and lets you inspect each block, including which bindings the selected visual uses. Open its block canvas to arrange and reconnect existing blocks with React Flow, or use the keyboard-accessible connection selectors. Invalid cycles are rejected; layout is temporary and connections are saved with Save applied preset. A built-in guide offers repeatable comparisons. Changes remain a draft until you choose Apply preset. Local files show a waveform overview before playback. Play explicitly states whether sound is enabled, playback position is visible, and reduced-motion mode offers Capture current picture. Pause holds your place, Resume continues, and a keyboard-accessible timeline lets you seek. An optional labeled frequency view shows the lower 0–2,000 Hz for local clips up to two minutes. Optional start/end controls repeat a short passage for comparison; Stop or manual seeking turns repeat off. Stop ends playback and the next Play restarts from the beginning. Start/stop is explicit; reduced motion supports still samples and audio playback requires opt-in. Audio stays in the browser and is never uploaded or sent to a model. Presets can be saved or exported. The optional sample walkthrough reveals each block’s input and output using a repeatable synthetic sample, with explicit next, restart and end controls. This is the browser learning stage; native macOS application-audio capture and Metal rendering are planned separately. See [visualizer scope](docs/slices/visualizer-lab.md).
