...
2. Create a task list (TaskCreate) with one task per slice plus "run stage benchmark" and "update docs". Dependencies in order.
...
Rules: never skip event logging or tests to "get the stage done". Ask before adding a dependency not listed in ARCHITECTURE.md.
