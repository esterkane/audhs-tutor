...
- Code editing: `features/code/CodeEditor.tsx` (CodeMirror 6 + plain textarea switch/fallback; Esc then Tab must always leave the editor). Only the four declared `@codemirror/*` packages may be imported (pnpm is strict: transitive packages do not resolve). The Pyodide runtime is served from `frontend/public/pyodide/` (`make pyodide`, pinned by `frontend/pyodide.manifest.json`); the worker must never load from a CDN.
