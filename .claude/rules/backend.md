...
- `models_ai/` — `provider.py` (`ModelProvider` interface + `TaskClass`), `ollama.py`, `claude.py` (LiteLLM SDK), `fake.py` (tests), `gateway.py` (`ModelGateway`: route → reserve budget → call → reconcile → `model_call` per attempt; P6), `budget.py` (reservations, `usage.py` cost view), `factory.py` (providers, installed-model discovery), `registry.py` + `downloader.py` + `manage.py` (HF/Ollama/MLX/fastembed artifacts, licence, disk, pull/bench/assign), `routing.py` + `routing_profiles.yaml` (TaskClass → registry id; learner overrides from `learner_preference`), `bench.py`, `budget.py`. Voice lives in `app/voice/` (P9): `vad.py`, `stt.py` (MLX Whisper via the ingest adapter), `tts.py` (Kokoro server client), `setup.py` (readiness/verify/activate), `loop.py` (WebSocket loop) — no `mlx.py` provider; STT/TTS calls still write `model_call` rows. Model names never appear outside the registry/profiles. Every model call — chat, embed, rerank, STT, vision — writes a `model_call` row with the registry id.
- `api/` — thin routers; `Depends(get_db)`, call orchestrator/kernel, return schemas. SSE for tutor stream, WS for voice (later).
- `db/` — `models.py` (SQLAlchemy 2 typed ORM, `Mapped[...]`), `events.py` (append-only writer, closed `Verb` enum, payload-key validation), `event_queries.py`, `traces.py`, `migrate.py` (`upgrade_to_head`), `portability.py` (learner export/wipe — portability, not recovery), `backup.py` (P5: versioned backup zip + restore into an empty target; `scripts/backup.py`, `docs/RECOVERY.md`), `ddl.py` (guard triggers). Every learner-scoped table has `learner_id`. IDs ULID, timestamps UTC.
- Structured LLM output via Instructor with a Pydantic model. Providers make **one** attempt (`max_retries=1`); the gateway logs it, asks for up to `MAX_REPAIRS=2` repairs (each attempt = one `model_call` row with `attempt` n, same `request_id`), then emits `invalid_output` and escalates one tier (P6).
