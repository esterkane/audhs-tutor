...
def main() -> int:
    ...
    sub.add_parser("list")
    s = sub.add_parser("search-hf"); s.add_argument("query"); s.add_argument("--gguf", action="store_true"); s.add_argument("--mlx", action="store_true")
    a = sub.add_parser("add"); a.add_argument("source"); a.add_argument("repo_id"); a.add_argument("--file"); a.add_argument("--tag"); a.add_argument("--role", default="chat")
    for name in ("pull", "bench", "rm"):
        sub.add_parser(name).add_argument("id")
    g = sub.add_parser("assign"); g.add_argument("task"); g.add_argument("id")
    args = p.parse_args()
    print(f"not implemented yet: {args.cmd} — implement in backend/app/models_ai/registry.py and call it from here", file=sys.stderr)
    return 2
